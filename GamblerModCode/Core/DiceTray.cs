...
public class DiceTray(int maxSize = 5)
{
    private readonly Queue<int> _diceTray = [];
    ...
    /// </summary>
    public async Task Populate(Player player)
    {
        while (_diceTray.Count < MaxSize)
        {
            int die = DiceRoll(player, 6);
            _diceTray.Enqueue(die);
    ...
    /// </summary>
    public async Task Repopulate(Player player)
    {
    ...
    /// </summary>
    public async Task Remove(Player player, int amount = 1)
    {
    ...
    public int DiceRoll(Player player, int dieSize)
    {
